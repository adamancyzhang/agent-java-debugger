...
description: 'On-demand extension content for agent-java-debugger: oinone/pamirs platform debugging (pamirs-designer, gql-driven backends) — gql protocol, login, model functions, and verified debugging recipes. Fetch it with `agent-java-debugger skills oinone` when debugging or driving an oinone app: call a model function via gql, log in, replay an export, or understand why an oinone request behaves unexpectedly. Not a separately loaded skill — costs no context until fetched.'
