...
description: 'Java JVM debugging CLI for AI agents. Attach to any JVM started with a JDWP port and debug it from the terminal or from agent scripts: breakpoints, stepping, stack frames, locals, and in-memory object inspection. Use when the user asks to debug a Java service, set a breakpoint, trace why a request behaves unexpectedly, inspect variables at a crash site, verify a code path with a real request, or debug anything on a JVM with a JDWP port. Triggers include requests to "set a breakpoint at ...", "step through this method", "is this method ever called", "where does this request go", "inspect this variable at the crash site". Emits machine-readable JSON events (--json). Observes only — never kills the target.'
allowed-tools: Bash(agent-java-debugger:*)
---

# agent-java-debugger

A JDWP CLI debugger for remote JVMs. Zero Python dependencies; pure stdlib. `npm i -g @adamancyzhang/agent-java-debugger` provides the `agent-java-debugger` command (the launcher finds Python automatically: `py -3`/`python` on Windows, `python3` elsewhere). Requires Python >= 3.10 on the CLI machine (the launcher verifies it); the debugged JVM can be anywhere — any OS, any CPU, nothing installed on it.
